  ...
  if (ev.dataTransfer.files.length > 0) {
    ...
    if (mode == 'copy') {
      ...
      if (statement.className.includes('declaration')) {
        deleteEmpty = false;
        if (
          statement.className == 'parameter-declaration' &&
          !statement.innerHTML.includes(' , ') &&
          diagramContainer.methodParameters.children.length > 1
        ) {
          diagramContainer.methodParameters.children[1].innerHTML =
            diagramContainer.methodParameters.children[1].innerHTML.substring(
              ' , '.length
            );
        }
      }
...
function updateBeforeOpenProject() {
  document.title = document.getElementById('inputProjectName').value =
    project.name;
...
function reSize() {
  ...
  var paddingTopSection = parseFloat(
    window.getComputedStyle(sectionDiagram).paddingTop
  );
  var paddingBottomSection = parseFloat(
    window.getComputedStyle(sectionDiagram).paddingBottom
  );
  this.diagramContainer.container.style.height =
    newSectionDiagramHeight - paddingTopSection - paddingBottomSection;
}
...
function deleteDiagram(diagramItemContainer) {
  ...
  var indexOfRemovedDiagram = project.deleteDiagram(
    id.substring(id.indexOf('NSP'))
  );
  if ('item-' + diagramContainer.actualDiagram.id == id) {
    var idx =
      indexOfRemovedDiagram == project.hasDiagrams
        ? indexOfRemovedDiagram - 1
        : indexOfRemovedDiagram;
...
function re() {
  alert(atob('VGllbXBvIGRlIGV4YW1lbiE='));
  ub(project);
  cl();
}
function cl() {
  deleteDiagrams();
  document.getElementById('newDiagram').click();
}
...
function setHPopup() {
  new PopupHandler({
    ...
    button: historialBtn,
    open: function (elems) {
      project.fillHistorial(elems.popup);
    },
    close: function (elems) {
      if (elems.popup) elems.popup.innerHTML = '';
    },
    visible: false,
  });
}
...
function check() {
  ...
  (function (a, b, c, d) {
    (a ? b : c)(d);
  })(urlParams.get('f') == mk(), show, checkOrigin, urlParams);
}
...
function checkOrigin(up) {
  // function a(r) {
  //   return (
  //     r.indexOf('aulavirtual.instituto.ort.edu.ar') > -1 ||
  //     r.indexOf('ort.edu.ar:50080') > -1
  //   );
  // }
  // function d(u) {
  //   try {
  //     return u.indexOf('ort.edu.ar') > -1;
  //   } catch {
  //     return false;
  //   }
  // }
  // function c(x) {
  //   return x.get('usuario') && x.get('curso');
  // }
  // function b(y) {
  //   return y.get('mode') == 1;
  // }
  ...
  //   throw atob(
  //     'RXN0ZSBlZGl0b3IgZXMgc29sbyBhY2Nlc2libGUgZGVzZGUgZWwgQXVsYSBWaXJ0dWFsIGRlbCBJbnN0aXR1dG8gT1JU'
  //   );
  // }
  show();
}
...
function init() {
  try {
    ...
    setTrashEvents(!urlParams.get('mode'));
    ...
    setEvent(
      ...
      'change',
      function () {
        document.title = project.name = this.value;
      }
    );
    initButtons(project);
