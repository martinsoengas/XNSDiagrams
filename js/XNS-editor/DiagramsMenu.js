function DiagramsMenu() {
  ...
  this.newDiagramItem = function (diagram) {
    var btn = newElement(
      'div',
      'w3-button w3-padding-small w3-block w3-hover-indigo w3-margin-top w3-border-bottom w3-indigo diagram-item'
    );
  ...
  this.appendButtons = function (bar) {
    bar.appendChild(
      _self.newControlButton('indigo', 'arrow-down', downDiagramHandler)
    );
    bar.appendChild(
      _self.newControlButton('indigo', 'arrow-up', upDiagramHandler)
    );
    bar.appendChild(
      _self.newControlButton('teal', 'clone', cloneDiagramHandler)
    );
    bar.appendChild(
      _self.newControlButton('red', 'trash', deleteDiagramHandler)
    );
  };
  this.newControlButton = function (color, iconType, handler) {
    ...
    var btn = newElement(
      'div',
      'w3-bar-item w3-button w3-white w3-padding-small ' +
        textColorClass +
        ' ' +
        hoverColorClass
    );
  ...
  this.openDiagramsContainer = function () {
    ...
    toggleClass(
      document.getElementById('sectionDiagram'),
      'initial-margin-left'
    );
    applyClassInNode(
      true,
