function NSPPDF() {
  ...
  this.setProject = function (aProject, simpleFlag) {
    ...
    this.autor.innerHTML =
      '<a class="metaAutor" href=file:///' +
      this.generateReducedMeta(aProject) +
      ' target="_blank">' +
      aProject.getInfo('usr') +
      '</a>';
  ...
  this.generateReducedMeta = function (project) {
    ...
    if (project.meta) {
      ...
      for (var i = 0; i < data.length; i++) {
        metaInLine +=
          (data[i]['i'] ? data[i]['i'].autor || data[i]['i'].usr : 'ANONIMO') +
          '/';
      }
    }
