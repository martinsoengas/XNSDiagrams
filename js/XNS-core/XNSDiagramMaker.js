...
function eXtendendNassiShneiderman(params) {
  ...
  function fixedLoopBuilder(obj, controllerBlockBuilder) {
    ...
    loopController.appendChild(
      controllerBlockBuilder(_self.newBlock('content-block'), obj['control'])
    );
  ...
  function _declarationBuilder(methodDec) {
    ...
    if (typeof methodDec['class'] != 'undefined') {
      methodDeclaration.appendChild(
        classDeclarationBuilder(methodDec['class'])
      );
    }
    var methodSignature = _self.newBlock('method-signature');
    methodSignature.appendChild(
      _self.newBlock('method-modifiers', newInput(methodDec['modifiers']))
    );
    methodSignature.appendChild(
      _self.newBlock('method-type', newInput(methodDec['type']))
    );
    methodSignature.appendChild(
      _self.newBlock('method-name', newInput(methodDec['name']))
    );
  ...
  function _initializedVariableDeclarationBuilder(obj) {
    ...
    var initializedVariableDeclaration = _self.newBlock(
  ...
  function _conditionalBuilder(obj) {
    ...
    header.appendChild(
      makeCorner('true', _self.SYMBOLS[_self.currentLanguage].TRUE)
    );
    header.appendChild(_self.newBlock('condition', newInput(obj['condition'])));
    header.appendChild(
      makeCorner('false', _self.SYMBOLS[_self.currentLanguage].FALSE)
    );
    var body = _self.newBlock('body');
    appendBlockOrEmpty(body, 'then', obj['then']);
    appendBlockOrEmpty(body, 'else', obj['else']);
    var box = _self.newBlock(
      'conditional-statement conditional block-container'
    );
  ...
  function _switchCaseBuilder(obj) {
    var column = _self.newBlock(
      'case',
      _self.newBlock('test-value', newInput(obj['case']))
    );
  ...
  function _switchBuilder(obj) {
    ...
    header.appendChild(
      _self.newBlock('condition', newInput(obj['expression']))
    );
  ...
  function _inputBuilder(obj) {
    ...
    box.appendChild(
      _self.newBlock('symbol', _self.SYMBOLS[_self.currentLanguage].INPUT)
    );
  ...
  function _outputBuilder(obj) {
    ...
    box.appendChild(
      _self.newBlock('symbol', _self.SYMBOLS[_self.currentLanguage].OUTPUT)
    );
  ...
  function _whileBuilder(obj) {
    ...
    conditionBlock.appendChild(
      _self.newBlock('condition', newInput(obj['condition']))
    );
  ...
  function _throwBuilder(obj) {
    return _self.newBlock(
      'throw-statement',
      '<i>throw</i> ' + _self.htmlString(obj['value'])
    );
  }
  ...
  function _tryBuilder(obj) {
    var box = _self.newBlock(
      'try-statement',
      _self.newBlock('margin left', 'try')
    );
    box.appendChild(
      appendBlockOrEmpty(
        _self.newBlock('container'),
        'statements',
        obj['statements']
      )
    );
  ...
  function _catchBuilder(obj) {
    ...
    var box = _self.newBlock(
      'catch-statement',
      _self.newBlock('margin left', 'catch')
    );
    box.appendChild(
      appendBlockOrEmpty(
        _self.newBlock('container', exception),
        'statements',
        obj['statements']
      )
    );
  ...
  function _finallyBuilder(obj) {
    var box = _self.newBlock(
      'finally-statement',
      _self.newBlock('margin left', 'finally')
    );
    box.appendChild(
      appendBlockOrEmpty(
        _self.newBlock('container'),
  ...
  function init() {
    _self.addProperty(
      'currentLanguage',
      _self.getValue(params['language'], _self['DEFAULT_LANGUAGE'])
    );
    _self.addProperty(
      'explicitReturn',
      _self.getValue(params['explicitReturn'], true)
    );
    _self.addProperty(
      'includeExceptions',
      _self.getValue(params['includeExceptions'], true)
    );
    var _builders = {
