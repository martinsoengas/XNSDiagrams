window.onload = function () {
  ...
  function insertAtCursor(elem, value, position) {
    if (position) {
      elem.value =
        elem.value.substring(0, position) +
        value +
        elem.value.substring(position, elem.value.length);
  ...
  function removeCharAtCursor(elem, position) {
    elem.value =
      elem.value.substring(0, position - 1) +
      elem.value.substring(position, elem.value.length);
  }
  ...
  var templates = {
    ...
    assignment:
      '{"type": "assignment", "data": {"variable": "[variableName]","value": "[value, variable or expression]" }}',
  ...
  function checkAndCompile() {
    ...
    try {
      ...
      var r =
        /Unexpected (token ([{,:])|string) in JSON at position ([0-9]+)/g.exec(
          e.message
        );
      if (r) {
        switch (r[1]) {
